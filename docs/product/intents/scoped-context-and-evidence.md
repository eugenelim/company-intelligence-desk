...
- Which scoping dimensions are required? Inception candidates: workspace, run,
  ...
  settled tuple.
- Which data should be immutable versus versioned?
...
- How are citation locators represented and verified, and do they survive
  evidence re-ingestion or re-indexing?
- What storage choices are sufficient without overbuilding?
...
Use `architect-design` to define context-service responsibilities, persistence
boundaries, evidence lineage, temporal rules, retrieval contracts, and
integration with the runtime and the workspace UI.
