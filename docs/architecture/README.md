...
| The privilege split | `migrations/versions/0002_*` | No application role holds `INSERT` on `events`. Three `SECURITY DEFINER` functions with disjoint `EXECUTE` grants, plus `fence_step` owned by `app_worker` so the narrow role gains no table access | r7 § Identity — two layers; `worker-runtime.md` change 1 |
