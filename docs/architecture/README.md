...
**What is designed and not built.** The authorization boundary's predicate, the
provider call, the run state machine's transitions, the browser stream, and the
Phase 1 measurements. Those belong to `walking-skeleton-authority-containment`,
...
[`role-configuration-seams.md`](role-configuration-seams/role-configuration-seams.md)
is the exception: its marker still reads "nothing here is built.
`src/ced/agents/` is empty", and both halves of that sentence are now false —
the compiler, the model seam and the toolset stack are in that package.
Correcting it needs the change that owns that file; until then this section is
