...
| The worker pool | `src/ced/worker/pool.py` | Claim under a lease with `SKIP LOCKED`, execute outside any transaction, heartbeat-renew fenced on epoch and owner. TTL 60 s, heartbeat 20 s, poll 30 s | r8 § 3 Runtime Model; `worker-runtime.md` § 6 Deployment and Operations |
