...
- [ ] **AC-0275.** Compiling a role fails unless the request its resolved model's adapter would issue carries a provider-level reasoning disable, derived from the compiled `thinking=False`. The decision is taken by calling the adapter's **own** request-building code for the resolved profile and reading what comes back — never a list of model ids and never a restatement of the adapter's rendering rule, since `CED_POOL_ALLOWED_MODEL_IDS` admits an id with no code change and a restatement drifts at the next version pin. It **fails closed** on a model whose adapter returns no disable, which on the pin refuses `claude-sonnet-4-6` and later, because whether an omitted field means reasoning-off at the provider is a fact this repository has not established. Role data cannot reach the decision: the compiler's admitted settings are `max_tokens`, `temperature` and `thinking`, and `walking-skeleton-role-compilation`'s AC-0269 refuses any key outside that set, so no role can supply an adapter-specific carrier that would suppress the rendering. **A role compiled where the pool wired no adapter is admitted, not refused** — the factory is unwired by default and a compile demanding a provider could not be exercised offline at all, the split `src/ced/agents/models.py` records — and that undecided case is carried to AC-0276. Deciding this needs a fourth `compile_role` argument; that function's own docstring reserves the addition for the spec that supplies it, and this is that spec. *Subject owner: `walking-skeleton-role-compilation`.*
- [ ] **AC-0276.** The executor refuses to issue a model call unless the request the resolved model would actually send carries a provider-level reasoning disable, **including the case AC-0275 admitted undecided because no adapter was wired**. The observable is that rendered request and not `ModelRequestParameters.thinking`: that field is assigned at one site, gated on the unified key surviving the merge, and a `False` there still renders nothing on an adaptive profile — so a criterion reading it would go green while the provider reasons, which is the defect this criterion inherits rather than one it may repeat. Reading the rendered request is also what makes the three adversarial routes decidable by one predicate: per-run settings, an attached thinking capability, and a caller-supplied adapter-native carrier each defeat the compiled value by a different path, and the first two never touch the rendered blob while the third never touches the unified key. The refusal sits below the per-step settings resolution, since the framework layers the agent's settings, then a capability's, then the per-run mapping, and a guard above that layering sees neither of the two it does not own; a guard sited on a wrapper model is insufficient for the same reason, because the wrapper forwards parameters untouched.
- [ ] **AC-0277.** Boot refuses an admitted model id whose adapter would render no disable, so a deployment naming such an id fails where the operator made the mistake rather than per-role at the first claim. This is decidable from the id alone and needs no wired factory and no provider call: the adapter's request-building code reads only the resolved profile. The decision is resolved in `agents/` and called from boot, because `tests/architecture/dependency_direction.py` admits a `pydantic_ai` name only in `agents/` and `adapters/` while `verify_boot` lives in `worker/`. AC-0275 stays the primary control, being the only place the *resolved per-role* model is known; this is the earlier one, and it is the one that makes admitting an id at deployment time a decision the system checks. *Subject owner: `walking-skeleton-foundation`.*
