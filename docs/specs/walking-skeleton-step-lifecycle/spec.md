...
- [ ] **AC-0275.** Compiling a role fails unless the model id it resolves to both carries an explicit `thinking=False` through to the resolved request parameters and has that value rendered by its adapter as a provider-level disable on the request the adapter would issue. The two halves are refused independently and the refusal names the model id and the half that refused. The decision is taken against the resolved profile and the resolved adapter's own output, not against a list of model ids: `CED_POOL_ALLOWED_MODEL_IDS` is a deployment-time value, so admitting an id needs no code change, and a criterion resting on an enumeration would pass while the newly admitted id reasons. The enforcement seam is the implementation's choice; the refusal is not.
- [ ] **AC-0276.** The `thinking` value the model reads is `False` on the executor's own run path, observed at the model boundary of a step the executor drove rather than of an agent the test constructed. The run path is the observation point because it is the only place the compiled value and every later settings layer meet: per-run settings are applied after the agent's, and an agent-settings override replaces the compiled mapping outright, so a value correct in the compiled role can still be wrong at the call. Demonstrated to hold against a run that presents the executor with a `thinking` value other than `False`.
...
- Technical: on `pydantic-ai` 2.45.0 the compiled `thinking=False` is dropped before the provider by four distinct layers, re-verified against the pinned package on 2026-09-22 by reading each construct whole. `Model.prepare_request` assigns the value only when the profile declares `supports_thinking` or `thinking_always_enabled`, and skips an explicit `False` on an always-thinking profile; the default profile declares neither flag and an unrecognised model-id prefix resolves to no profile, so the larger set of ids never carries the value at all. `BedrockConverseModel` renders `thinking: {type: disabled}` only on the non-adaptive Anthropic branch — the adaptive branch has no `else` and emits nothing for `False`, and the current Claude families take the adaptive branch — and a caller-supplied `thinking` key in the additional-fields blob wins over both. At the call, per-run settings are merged over the agent's and an agent-settings override replaces the agent mapping outright and suppresses the per-run one. **An earlier record called the Bedrock path safe; that read one branch of an if/elif and is false**, which is why AC-0275 and AC-0276 state a property and enumerate no mechanism (source: `walking-skeleton-role-compilation` § Follow-ons and its T3 security adjudication; re-verified 2026-09-22).
