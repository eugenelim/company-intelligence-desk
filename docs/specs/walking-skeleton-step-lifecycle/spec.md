...
- **TDD (AC-0226, AC-0227, AC-0228, AC-0229, AC-0230, AC-0231, AC-0232, AC-0237, AC-0241, AC-0245)** — the persistence and deadline criteria, because each is a compressible invariant with a cheap oracle and no provider in the loop. AC-0232 uses a `Model` stub that hangs, so the deadline path is exercised with no spend. AC-0227 is exercised across a **process boundary**, because same-process reuse would pass on in-memory state the design forbids relying on.
...
- [ ] **AC-0245.** A resumed step's entitlements conjunct is evaluated against the initiating user's current entitlements, not against a snapshot carried in the persisted bytes, so revoking an entitlement takes effect on a step already waiting for approval.
...
- eugenelim: `workspace.toml` `[backlog].open` — **the DR5 reject-and-resume cycle cap has no acceptance criterion anywhere.** r5 § 3 Runtime Model returns the approver's reason as the deferred tool result and the agent revises; r5 names that shape as the negotiation the design forbids, with the three-cycle cap as its sole bound. `walking-skeleton-evidence/plan.md` dispositions DR5 to its own T1 as "lands, unmeasured", and that spec owns the reject-and-resume transition the cap counts — so the criterion belongs there and adding one is an amendment to an Approved spec, out of this change's scope. Until then the cap can ship absent with every gate green. **AC-0244 is retired unused** — it was drafted here and moved before approval; the identifier is not reallocated.
