...
- [ ] **AC-0275.** Compiling a role fails unless the request its resolved model would actually send carries a provider-level reasoning disable. The probe begins at the compiled `ModelSettings(thinking=False)` and runs the resolved model's **own full settings-to-request resolution**, not its renderer alone: the step that discards an explicit `False` on an always-thinking profile, and the one that never carries it on a profile declaring neither flag, both live in that resolution, so a probe handed a hand-built parameters object skips two of the recorded defeat layers and is blind in exactly the way the next version bump will expose. It runs against the **adapter instance the deployment resolved**, never one the check constructs — constructing a Bedrock adapter needs a region name or a client, which no configuration here supplies — and it never restates the adapter's rendering rule, which drifts at the next pin. It **fails closed**: a resolved model whose request carries no disable is refused, which on the pin refuses `claude-sonnet-4-6` and later, because whether an omitted field means reasoning-off at the provider is a fact this repository has not established. **Two cases are admitted, and neither may be inferred from an unresolved profile.** A role compiled where the pool wired no adapter is admitted, the factory being unwired by default and a compile demanding a provider being unexercisable offline — the split `src/ced/agents/models.py` records — and that case is carried to AC-0276. A model the deployment **declares** non-provider-backed, as `stub:counting` is in `deploy/compose.yaml`, is admitted because it reaches no provider. The declaration must be explicit: an id resolving to no profile is refused, not admitted, since an unrecognised real model id resolves to no profile too and treating that as a fixture would install one of the recorded defeat layers as policy. Role data reaches none of this — the compiler's admitted settings are `max_tokens`, `temperature` and `thinking`, and `walking-skeleton-role-compilation`'s AC-0269 refuses any key outside that set. Deciding this needs a fourth `compile_role` argument, an addition that function's own docstring reserves for the spec that supplies it. *Subject owner: `walking-skeleton-role-compilation`.*
- [ ] **AC-0276.** No call reaching a provider is issued unless the request the resolved model would send carries a provider-level reasoning disable, **including the case AC-0275 admitted undecided because no adapter was wired**. The predicate binds to **every path on which a step reaches the provider**, streaming and non-streaming alike, since the adapter builds its request at two sites and a guard on one leaves the other open for the spec that adds the browser stream. The observable is the rendered request rather than `ModelRequestParameters.thinking`: that field is assigned at one site, gated on the unified key surviving the merge, and a `False` there still renders nothing on an adaptive profile, so reading it would go green while the provider reasons — the defect this criterion inherits and may not repeat. **The guard sits at the model seam, not at the executor's call site.** The framework resolves the agent's settings, a capability's and the per-run mapping into one mapping *before* it calls the model, so a model-seam guard receives exactly what the adapter will and can re-derive the rendered request from it, while a guard at the executor's call site sits above that layering and sees neither of the two routes it does not own. Demonstrated against all three routes — per-run settings, an attached thinking capability, and a caller-supplied adapter-native carrier — each defeating the compiled value by a different path, and one predicate over the rendered request decides them all.
...
Allocated in this spec and never reallocated. Each is already stated as
withdrawn in the prose that retired it; listing them here is what makes the
