...
- **TDD (AC-0226, AC-0227, AC-0228, AC-0229, AC-0230, AC-0231, AC-0232, AC-0237, AC-0241, AC-0245, AC-0248, AC-0253, AC-0254, AC-0222, AC-0242, AC-0255, AC-0256, AC-0263, AC-0264, AC-0275, AC-0276, AC-0277)** — the persistence and deadline criteria, plus the four quarantine criteria received from `walking-skeleton-role-compilation`. **AC-0222 and AC-0242 carry the `substrate` marker** — the first needs the database round trip that is its whole point, the second searches the run's events and the payload objects they reference — while AC-0255, AC-0256 and AC-0263 run offline and AC-0264 carries the marker, since it is a write the database refuses. **AC-0275, AC-0276 and AC-0277 run offline and reach no provider.** Resolving a profile is pure given a model id. **Asking the adapter what it would send is not** — it needs a model instance. **The check stays offline and carries no `substrate` marker:** it constructs the adapter directly from the framework rather than through the deployment's factory, and request-field building reads only the profile and the settings. Restating the adapter's rendering rule inside the check is refused however convenient: that is an enumeration of adapter behaviour under another name, it is the thing AC-0275's "never against a list of model ids" clause exists to prevent, and it drifts silently at the next version pin. AC-0276 reads the value at the model boundary off a stub the executor drove. None of the three may be demonstrated on an agent the test built, because an agent the test built is exactly the construction that left AC-0204 green while the provider reasoned.

**Stub coverage:** AC-0255 carries a validated red stub against the parser seam; AC-0222, AC-0242 and AC-0256 carry `no stub (implementation-discovered)` with T5's discovery predicate. **AC-0275 and AC-0277 carry validated red stubs** — `compile_role` and `verify_boot` both exist and are public, so neither seam waits on discovery — and **AC-0276 alone carries `no stub (implementation-discovered)`**, its seam being the executor, which does not exist. Each is a compressible invariant with a cheap oracle and no provider in the loop. AC-0232 uses a `Model` stub that hangs, so the deadline path is exercised with no spend. AC-0227 is exercised across a **process boundary**, because same-process reuse would pass on in-memory state the design forbids relying on.
...
- [ ] **AC-0275.** Compiling a role fails unless the request its resolved model's adapter would issue carries a provider-level reasoning disable. **Whether the unified `thinking=False` suffices is a per-family fact** — see § Assumptions, which owns the measurement: it renders a disable on `claude-sonnet-4-5` and earlier and renders nothing on `claude-sonnet-4-6` and later. **Which families Phase 1 admits, and therefore whether a provider-native carrier is needed at all, is an open owner decision recorded in § Follow-ons; this criterion's text is provisional on it.** Role data still cannot supply the carrier; `walking-skeleton-role-compilation`'s AC-0269 closes the declared key set and is unchanged. The decision is taken against the resolved profile and the adapter's **own** request-building output, never a list of model ids or a restatement of the adapter's rendering rule: `CED_POOL_ALLOWED_MODEL_IDS` admits an id with no code change, and a restatement drifts at the next version pin. It fails closed on a model whose adapter emits nothing, because whether an omitted field means reasoning-off at the provider is a fact this repository has not established. **A role compiled where the pool wired no adapter is admitted, not refused** — the factory is unwired by default and a compile demanding a provider could not be exercised offline at all, the split `src/ced/agents/models.py` records — and that undecided case is carried to AC-0276 rather than dropped. Compiling against an adapter needs a fourth compiler argument; `compile_role`'s own docstring reserves that addition for the spec that supplies it, and this is that spec. *Subject owner: `walking-skeleton-role-compilation`.*
- [ ] **AC-0276.** The executor refuses to issue a model call whose resolved `thinking` is anything but `False`, **including the case AC-0275 admitted undecided because no adapter was wired**, and the value the model reads on the executor's own run path is `False`. The read is pinned to `ModelRequestParameters.thinking` as the model's `request()` receives it, after `prepare_request`: that call strips the key from the settings mapping whether or not it carried the value across, so a check reading the settings the model was invoked with asserts nothing on any profile. The refusal sits below the per-step settings resolution, because the framework layers an agent's settings, then a capability's, then the per-run mapping, and a guard above that layering cannot see the two it does not own. The stub it reads off must be a model AC-0275 admits, which ties the stub's profile to the criterion beside it rather than to the test author's convenience — the difference from the stub AC-0204 was demonstrated on. It is a refusal and not only an observation because a test pins the call path it was written against while the defeating layers are things a later caller adds. Demonstrated against a run presenting the value three ways — per-run settings, an attached thinking capability, and the adapter's provider-native carrier — since each beats the compiled value by a different route and one case passes on the two it omits.
- [ ] **AC-0277.** Boot refuses an admitted model id whose resolved profile and adapter cannot be made to carry the disable, so a deployment that admits such an id fails where the operator made the mistake rather than per-role at the first claim. `CED_POOL_ALLOWED_MODEL_IDS` is deployment-time and needs no code change, and `verify_boot` already validates it before opening a connection. The capability decision itself is resolved in `agents/`, which boot calls: `tests/architecture/dependency_direction.py` admits a `pydantic_ai` name only in `agents/` and `adapters/`, and `verify_boot` lives in `worker/`. AC-0275 stays the primary control, being the only place the *resolved per-role* model is known; this is the earlier one. *Subject owner: `walking-skeleton-foundation`.*
...
- eugenelim, **open decision blocking AC-0275's final wording** — *which Claude families Phase 1 admits.* The unified `thinking=False` renders a provider-level disable on `claude-sonnet-4-5` and earlier and renders nothing on `claude-sonnet-4-6` and later, measured against the pin on 2026-09-22 and recorded in § Assumptions. Two routes follow and they differ in cost. **Restrict the admitted set to the families where the unified setting reaches the wire**, and AC-0275 stays a refusal over the resolved profile and adapter with no new machinery, no provider knowledge in `agents/`, and no change to AC-0269 — at the price of Phase 1 not running Sonnet 5 or Opus 5 on Bedrock. **Or have the compiler supply a provider-native carrier** for the adaptive families, which reaches the wire but puts a Bedrock-specific key in `agents/`, contradicting r5 § 2 R3 and `src/ced/agents/models.py`'s recorded "nothing here knows what a Bedrock model is", and needs a further refusal so role data cannot supply the same key. The second is a ratified-decision change and is Ask-first under this spec's own § Boundaries. Until it is taken, AC-0275's carrier sentence is provisional.
