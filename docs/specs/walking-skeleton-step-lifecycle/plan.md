...
deploy/compose.yaml, tests/thinking_reaches_the_model/**
...
- **The non-provider declaration lands here too**, as a pool configuration value separate from `CED_POOL_ALLOWED_MODEL_IDS`, which is why `pool.py` and `compose.yaml` are in Touches. Reading allow-list membership as the declaration admits every id that reaches compilation, so the discriminating test is that `stub:counting` compiles while declared and the *same* id fails once the declaration alone is removed — with the allowed set left intact, so the refusal cannot be AC-0251's.
...
**Touches:** src/**/adapters/**, src/**/agents/models.py, src/**/worker/executor.py, tests/thinking_reaches_the_model/**

**Tests:**
- AC-0276 asserts no call reaches a provider unless the model actually invoked would send a disable, driven four ways: per-run settings, an attached thinking capability, a caller-supplied adapter-native carrier, and a model substituted after compilation. One predicate over the rendered request decides all three; reading `ModelRequestParameters.thinking` is refused as the observable, being green on the third route and on every profile declaring neither flag.
...
- **Every model method reaching the provider is asserted**, which on the pin is the request path and `count_tokens`. Streaming and non-streaming both funnel into one build site and are not the distinction that matters; `count_tokens` is the genuinely separate one, a wrapper forwards it unguarded, and ADR-0006 D1's return condition plans to switch it on.
...
- **Mutation proof:** disable the guard and record which of the four routes red and which stay green, per route rather than in aggregate.
...
**Done when:** AC-0276 is green under `pytest -m 'not substrate'` with all four routes driven on every provider-reaching method, and the ledger records the mutation result per route.
