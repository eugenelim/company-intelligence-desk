...
- AC-0273 feeds the loader an `integration_registry` row for each of the four refusal shapes its criterion names — `tools` absent, `null`, a non-array, and `[]` — and asserts each fails to load naming the row. Three are decided offline on the decode seam. **The fourth is an unbound row and is decided against `list_integration_tools()` under the `substrate` marker**, because an unbound row is by definition absent from any one role's pinned rows, and the ratified design puts the `tools` rule on what that function reads while stating it must read the tables. That row is the one a bound-only guard would miss and the one that otherwise shrinks AC-0233's enumeration rather than failing anywhere, so it is the case worth writing first.
...
- **At least one per-key violation and one `count_tokens_before_request` violation are asserted through `verify_boot` itself**, offline, not only through `validate_pool_config`. AC-0265 and AC-0270 name `verify_boot` as the failing callable, and nothing otherwise fails if the boot path validates inline and never calls the new one — the same delegation gap this plan names for `load_role` and repairs with a real-callable check, and the one that would leave ADR-0006 D1's mitigation remembered rather than checked on a `restart: "no"` fleet. This is reachable offline precisely because the refusal raises before either connection is attempted; only the admitted-configuration return needs `validate_pool_config` directly.
...
- **The environment parse is its own callable, `validate_pool_config(env) -> PoolConfig`, and `verify_boot` calls it first.** That split is what puts AC-0265 and AC-0270 in the offline gate, and the split is the point rather than the ordering alone: `verify_boot` (`src/ced/worker/pool.py:167`) opens the worker and policy `psycopg` connections and has no return before them, so **calling `verify_boot` offline with a valid configuration reaches the database and raises — the admitted-configuration case is not observable through `verify_boot` at all.** **The refusal cases are decided through `verify_boot`, which is the callable AC-0265 and AC-0270 name**, and they stay offline because the refusal raises before either connection is attempted. Only the admitted-configuration case is decided against `validate_pool_config` directly, asserting it returns a `PoolConfig` carrying the parsed values with no connection attempted — calling `verify_boot` offline with a valid configuration reaches the database instead, so that case is observable nowhere else. A malformed pool configuration should fail before the process reaches for a database in any case, which is why the parse is worth separating rather than only sequencing.
- **One substrate check joins the migration to the loader**, because nothing else does: it inserts a role row and its pinned registry rows, reads them back through the real `load_role`, and asserts the model id, the four declarable limits, the output-contract name, the ceiling, and the registry columns the compiler reads — `tools` and `pool_classes` in particular, since `pool_classes` is otherwise read only by AC-0258, which runs offline against a record and so cannot catch a name the migration and the loader spell differently. **The same check drives one malformed record through the real `load_role` and asserts it is refused**, because AC-0251's omitted-`model_id` clause, AC-0262, AC-0266 and AC-0273's bound-row shapes all say *fails to load* and are otherwise decided only on the decode seam: a `load_role` that parses inline and never calls that seam ships with all four green. AC-0273's unbound-row case needs no such backstop, being decided against the real `list_integration_tools()` already. The malformed record must be one of those four shapes, so its refusal can only come from the decode seam — a shape the query itself rejects never reaches it and would establish nothing. The `tests/schema/` check names columns and the offline loader criteria are fed a record, so a column the migration spells one way and the loader reads another way is green on both sides and first observed as a step failing at compile time in production. This is the design's own inspectability verification, "a loader test asserting all four from one row".
...
- AC-0274 feeds the parser a value of a type outside the declared admitted-scalar set; a second case whose type is an admitted enumeration but whose value is not a member of it; and **a third that is a well-formed `<owner_scope>/<content_hash>` locator, asserted refused because Phase 1 mints none.** All three assert refusal, and the third is deliberately a blanket refusal rather than a provenance check: with no minting source in this spec there is no minted locator to pair it with, and a refusal-only provenance case would pass identically against a parser that rejects every locator categorically. The case to write is therefore the categorical one, and the assertion should say so, so that the spec which later adds a minting source has to change this test rather than inherit a green one that never checked provenance. The declared set is r8 § 4's and lives beside the label vocabulary as a module-level constant; the suite reads the constant for the same reason AC-0268's does. The negative case must not be free prose — AC-0220 already covers that, and a scalar-shaped value is the only thing that distinguishes this criterion from it.
...
- 2026-09-21: amendment closing the sustained findings of the round-7 pre-EXECUTE review, the first round in which no lens reported a blocker. Both were leftovers of the round-6 repair rather than new gaps: T2's `Tests` field had stated two canonical homes for AC-0265's and AC-0270's refusal cases, since the bullet explaining the `validate_pool_config` split survived unchanged beside the new bullet requiring the refusals through `verify_boot`; and the content-addressing Follow-on named `spec.md` as its register anchor while the entry sits on `plan.md`. The refusal cases are now stated once, against `verify_boot`, the callable both criteria name, with only the admitted-configuration case decided against `validate_pool_config`.
- 2026-09-21: amendment closing the sustained findings of the round-6 pre-EXECUTE review. AC-0273's unbound-row case is decided against `list_integration_tools()` under the `substrate` marker rather than offline on the decode seam: an unbound row is absent from any one role's pinned rows by definition, and the ratified design puts the `tools` rule on what that function reads while stating it must read the tables. The three bound-row shapes stay offline. AC-0265 and AC-0270 now assert at least one per-key and one `count_tokens_before_request` violation through `verify_boot` itself, because they name it as the failing callable and nothing otherwise failed if the boot path validated inline and never called `validate_pool_config`. AC-0274's third clause states the Phase 1 blanket refusal instead of a provenance case that could not be paired. **Two adjudications of the same AC-0273 substance reached opposite verdicts this round**; the controller resolved it from the ratified design, which settles it on a third ground that does not depend on the disputed premise.
- 2026-09-21: amendment closing the sustained findings of the round-5 pre-EXECUTE review. Both blockers were regressions the round-4 amendment introduced. AC-0274's governing-set ruling had relocated the locator channel rather than closing it, and now binds on behaviour rather than class; AC-0273 was scoped to bound rows while the rule it contracts and the enumeration it protects both span every row. The design's loader-stage event type failed the CHECK quoted in the same sentence for the second time — `load_role`, then `role.load_failed` — so the design now states the pattern as the constraint and says to check a proposed type against it rather than against an example. `validate_pool_config` is named as the callable AC-0265 and AC-0270 are decided against, because `verify_boot` has no return before its connections and the admitted-configuration case is not observable through it at all.
...
- 2026-09-22: **plan re-approved by eugenelim.** The build strategy is unchanged — T1 through T4, three stacked PRs, T1 complete and pinned with evidence `git:9a6d041`. The amendment touched no task's `Touches`, `Tests` obligation or `Done when`, so rescheduling emits T2, T3 and T4 with T1's preserved dependency treated as met.
- 2026-09-22: **spec re-approved by eugenelim** over the amended contract. The scope decision covers AC-0204's rewording to the resolved request parameters, the one `§ Follow-ons` obligation deferring the demonstration that the disable reaches the model, and the stub reformat. Five adjudicated review rounds ran; the owner directed the build to proceed after the round-13 adjudication found no sixth round justified.
- 2026-09-21: amendment reformatting T2's AC-0202 stub, authorized by the owner and recorded in [`notes/amendment-2026-09-21-stub-formatting.md`](notes/amendment-2026-09-21-stub-formatting.md). **The stub block, whitespace only; AC-0204, reworded to name the observation point the pinned framework actually exposes, on the owner's direction of the same day; and the `§ Follow-ons` bullet and register movements that closing the resulting security findings required, enumerated in the amendment note. No `Touches`, `Tests` obligation or `Done when` changes, and no criterion other than AC-0204 changes.** `ruff format --check .` is the first gate in `AGENTS.md`, this ruff formats fenced Python inside Markdown, and the stub's inline `role={...}` literal was the only thing in the repository the gate refused — `origin/main` is clean, `walking-skeleton-step-lifecycle`'s plan carries a Python fence that passes, and the failure predated any implementation code. The amended text is what `ruff format` produces, which is also byte-identical to the stub as materialized in `tests/compiler/test_stack_composition.py`, whose red was proven against the pre-amendment text. So the plan now agrees with what shipped, and materializing a stub byte-identically no longer conflicts with passing the style gate — a conflict every future spec pinning a stub would inherit. T1 was pinned complete with evidence `git:9a6d041` by the same transition; its section is unchanged.
- 2026-09-21: **spec re-approved by eugenelim** after eight adjudicated pre-EXECUTE review rounds. `Status: Approved` was already in force from the 2026-09-20 gate; this entry records the scope decision holding over the amended contract. Five criteria were added (AC-0266 through AC-0270) and two more in the rounds after (AC-0273, AC-0274); AC-0238's oracle, AC-0261's reach, AC-0273's scope and AC-0274's third clause changed; AC-0271 and AC-0272 were allocated and withdrawn without reallocation. Five owner decisions are baked in: the `Protocol`-typed model seam in `worker/`, ratifying `role-configuration-seams`, the follow-on-plus-register carrier for ADR-0006 D1 and D2, binding content-addressing values by behaviour rather than class, and the Phase 1 blanket refusal in AC-0274's third clause.
- 2026-09-21: **plan re-approved by eugenelim** — the build-strategy decision over the amended plan. `Status: Approved` was already in force from the 2026-09-20 gate. The pinned fields changed substantially across the review: T2's `Tests` gained AC-0201's own cases, AC-0206's omitted-key clause, per-key AC-0265 and AC-0270 boot cases, the closed-set cases for AC-0266, AC-0267 and AC-0269, AC-0273's four `tools` shapes and a substrate migration-to-loader round-trip; T3 gained AC-0268, AC-0274 and a committed candidate-set baseline; `Touches` gained `tests/fixtures/candidate_set_expected.json`; and both `Done when` lists were re-cut so each criterion sits in a gate that can decide it. `decode_role_record` and `validate_pool_config` are named as the seams the offline criteria call. Delivery stays three stacked PRs — T1+T2, T3, T4 — with T3 DEEP and T2 MIXED as § Rollout records.
