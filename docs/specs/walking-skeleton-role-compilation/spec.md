...
- **Designed by:** [`role-configuration-seams`](../../architecture/role-configuration-seams/role-configuration-seams.md), which closes `agent_role` and `integration_registry` to r5's shapes and names the compiler's seams. Every criterion here reads a field that design provides.
...
- **TDD (AC-0201, AC-0202, AC-0203, AC-0204, AC-0205, AC-0206, AC-0219, AC-0251, AC-0220, AC-0221, AC-0222, AC-0233, AC-0234, AC-0238, AC-0246, AC-0250, AC-0258, AC-0259)** — all of it, because all of it is a compressible invariant with a cheap oracle and no provider in the loop. The framework ships `TestModel` and `FunctionModel`, so the compiler, the toolset stack and the quarantine boundary are all exercised without a model call. **AC-0233 carries the `substrate` marker**, because it enumerates by reading `agent_role` and `integration_registry` and an in-memory fixture would be the suite-held list the criterion forbids. Every other criterion runs in the offline gate, decided against the compiler, the constructed stack or the parser called directly.
...
**No Phase 1 role can hold a `free-text` integration.** AC-0203 refuses one on
...
| Enforce the compile-time role guards | AC-0203, AC-0204, AC-0205, AC-0206, AC-0251, AC-0258 | Each discharges a named upstream rule: AC-0203 r5 § 2's R2 (refuse a `free-text` integration on a non-quarantined role), AC-0204 R5's `thinking` disabled and DR8, AC-0205 DR9 — which r5 calls the control that stops a framework-conducted retry loop over attacker-authored filing text — AC-0206 R5's `effective limits = min(pool default, role value)` under § 4's compile-failure semantics, and AC-0251 R5's model-id-within-the-pool's-allowed-set. R5 is a list of six invariants, not one, so each is cited by the member it discharges; with AC-0251 the only member of that catalogue row left unasserted is `ceiling ⊆ parent`, deferred as `may_run`. R5's compile-time ceiling re-verification duty, stated outside the row, is a separate Follow-on | Settled decisions ship as prose with nothing asserting them |
...
- [ ] **AC-0201.** Compiling a role fails when a tool would be reachable outside the wrapped stack. Two cases: a `ceiling` entry naming an integration the compiler was not given, and a `tool_name` absent from that integration's `tools`. Both are compile errors, not denials at call time.
...
- [ ] **AC-0204.** The compiler sets `model_settings.settings.thinking` to `False` on every compiled role, and compiling a role that declares any other value fails. Omitting the key is not sufficient: `ThinkingLevel` omitted leaves the provider default, which reasons on reasoning models.
...
- [ ] **AC-0258.** Compiling a role fails when any integration its `ceiling` binds does not list the role's `pool_class` in `integration_registry.pool_classes`, an empty list meaning available to every class.
- [ ] **AC-0219.** A role is quarantined exactly when its `ceiling` is empty, and a compiled quarantined role carries no domain tools on its stack and the `reference-selection` output type. Both are read from the compiled agent, not the record; a quarantined role declaring any other `output_schema_ref` fails to compile.
...
- [ ] **AC-0238.** The candidate reference set is complete before the quarantined agent's first model turn, and the set observed before that turn is identical to the set observed after the last one. Both snapshots are taken by the test's stub model at its own request boundary, so the ordering is decided outside the implementation.
- [ ] **AC-0250.** A mutation of the candidate reference set attempted while the agent run is in flight is refused. The set is per-step in-process state, so the refusal is asserted in process and not against a store.
...
- eugenelim: this spec § Acceptance Criteria — **AC-0222 is universally quantified and proven on one path.** One quarantined step, one planning step, one fixture. The Testing Strategy caveats adaptive adversaries but not path coverage, so a reader takes a single-path result as universal.
