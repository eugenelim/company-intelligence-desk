...
- eugenelim: `workspace.toml` `[backlog].open` — **R5's compile-time ceiling re-verification has no criterion.** r5 § 4 gives R5 a duty outside its catalogue row: re-verify the ceiling predicates against the pinned `arg_schema`. AC-0248 covers the resume case; nothing covers the ordinary compile case, and the authoring-time refusals fire at authoring rather than at compile.
