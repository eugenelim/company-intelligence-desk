...
- **TDD (AC-0201, AC-0202, AC-0203, AC-0204, AC-0205, AC-0206, AC-0219, AC-0220, AC-0221, AC-0222, AC-0233, AC-0234, AC-0238, AC-0242)** — all of it, because all of it is a compressible invariant with a cheap oracle and no provider in the loop. The framework ships `TestModel` and `FunctionModel`, so the compiler, the toolset stack and the quarantine boundary are all exercised without a model call. AC-0222 is exercised by an **integration** test: a quarantined step and a planning step together, across the database, because the indirect path is the one a unit test cannot see.
...
- [ ] **AC-0238.** The candidate reference set is complete before the quarantined agent's first model turn and does not grow during the run, observable as an ordering fact from the recorded mint and a post-run mutation attempt that fails.
- [ ] **AC-0242.** A refused integration result records its rejection without the rejected text reaching the event log.
