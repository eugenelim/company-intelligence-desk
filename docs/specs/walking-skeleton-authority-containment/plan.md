...
- AC-0252 injects a connection-level failure rather than a serialization failure, with the lease intact, so the worker cannot attribute it, and asserts the worker attempts termination. The fence is the foundation's and correct; what this observes is that the caller passes the epoch it holds rather than re-reading the current one.
