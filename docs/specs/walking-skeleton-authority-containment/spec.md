...
- eugenelim: this spec § Acceptance Criteria — **a `url` argument's scheme is unconstrained by any criterion.** AC-0240 requires a host-constraining predicate and nothing requires a scheme-constraining one, so `file://sec.gov/etc/passwd` satisfies `host_eq("sec.gov")` while every resolver ignores that authority — the predicate AC-0240 forces to be present decides nothing about what the callee opens. T1 refuses such a declaration at authoring time, fail-closed and **beyond any criterion**; the strengthening needs ratifying or reversing. Found by the implementation security review of 2026-09-23.
- eugenelim: this spec § Acceptance Criteria — **a call may omit an argument the ceiling constrains, and no criterion decides what happens.** AC-0316 and AC-0317 close an argument no predicate ranges over; the mirror — a predicate no argument arrives for — is unowned, and deciding only the arguments a call supplies makes an empty call an admission against any ceiling. T1 denies an incomplete call, fail-closed and **beyond any criterion**, which also means AC-0218's two admitting values travel in one call rather than two. Needs ratifying or reversing. Found by the implementation security and quality reviews of 2026-09-23.
- eugenelim: this spec § Acceptance Criteria — **`within(root)` has no analogue of AC-0215's public-suffix refusal.** `within("/")` is present, satisfies AC-0316, and admits `/etc/passwd`. T1 refuses that one root, **beyond any criterion**; a general rule for a root too shallow to bound anything is not attempted and has no owner.
