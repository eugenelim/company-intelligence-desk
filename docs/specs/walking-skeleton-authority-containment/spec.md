...
carries no spend. AC-0209, AC-0211, AC-0212, AC-0236, AC-0239 and AC-0243 reach the
database, which the foundation spec's local substrate supplies.
...
| Record a denial | AC-0209 | r5 § 4 Contracts and Invariants' invariant table makes the policy-decision append a named control — a `policy.decision` commits before the action it authorizes, under a live fence — but its § 10 Rollout criterion 5 asks only that a *failed* append stop the body. Nothing upstream asks that a *refusal* be recorded, which is what attributability rests on | A refused call leaves no attributable trace, and the attributable-action goal is unmeasurable |
...
- [ ] **AC-0239.** A decision point whose step lease has been taken by another worker refuses the call, commits no `policy.decision`, and does not execute the tool body; the evicted worker abandons the step to its new owner rather than failing it.
- [ ] **AC-0240.** Declaring a `url`-typed argument with no host-constraining predicate is refused at authoring time.
- [ ] **AC-0243.** With the `policy.decision` append forced to fail on its own connection inside the decision point, on a call the installed predicate **admits**, the tool body does not execute.
