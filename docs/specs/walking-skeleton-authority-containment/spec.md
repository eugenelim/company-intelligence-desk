...
| Decide the undecidable append failure | AC-0252 | AC-0211 and AC-0243 terminate the step, AC-0239 abandons it, and the discriminator is whether the fence was held. That is knowable from the serialization failure `append_policy_decision` raises and not from a connection-level failure, where the worker cannot tell whether the transaction reached the fence at all. Without a stated direction the exception handler picks one by accident | The three criteria partition the space they name and leave a real state outside it, decided by catch order |
...
- [ ] **AC-0239.** A decision point whose step lease has been taken by another worker refuses the call, does not execute the tool body, and leaves no committed `policy.decision`, the fenced transaction having rolled back. The evicted worker abandons the step to its new owner rather than failing it.
- [ ] **AC-0252.** An append failure the worker cannot attribute to fence loss is treated as fence-held: the step terminates, as AC-0211 and AC-0243 require. A connection-level failure cannot tell the worker whether the transaction reached the fence, so the fail direction is stated rather than left to whichever exception the handler catches first.
...
- [ ] **AC-0209.** A call refused under a live fence commits a `policy.decision` event recording the denial, the acting agent role, and the initiating principal.
