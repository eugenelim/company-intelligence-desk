...
because with no write path open there is nothing to reject it in favour of, and
because the derived idempotency key is what makes the re-execution it leads to
benign. The
...
type: `ced.domain.events` declares `run.requested`, `run.cancelled`,
`tool.invoked`, `tool.completed`, `role.load.failed`, `role.compile.refused`
and `policy.decision`, and `TERMINAL_EVENT_TYPES` is run-scoped. `release`
...
above the layer that would append one. So on exactly the paths where the
`policy.decision` append failed, the event log holds nothing about the
authorization decision. That is the repudiation case r5 § 7 prices, reached by
...
- **Every ceiling entry `src/ced/agents/ceilings.py` builds is built through `declare`**, the containment fragment's authoring surface, and an entry `declare` refuses is refused at compile time rather than handed to `evaluate`. This is not an implementation preference. `evaluate` re-checks exactly three things — argument coverage, an argument no predicate ranges over, and an unrecognised domain type — while `declare` holds six refusals that `evaluate` does not repeat: a prefix on an interpreted type, a non-expressible predicate, `host_in_domain` over a public suffix, `within(/)`, a `url` argument with no host-constraining predicate, and a `url` argument whose `scheme_in` is absent or escapes `RESOLVABLE_SCHEMES`. `CeilingEntry` is a directly constructible frozen dataclass and `agent_role.ceiling` is bare `jsonb` with no CHECK, populated by operator insert, so a decoder that built the dataclass itself would admit a stored row encoding `scheme_in{file}` beside a host prefix with AC-0207, AC-0210, AC-0235, AC-0236 and AC-0318 all green. That is the fail-closed widening this spec's second `Never do` forbids, reached through the one path § Follow-ons names as the only enforcement point there is. A check drives a stored row violating each of the six and asserts compilation refuses.
...
**Done when:** the grep gate returns no misattributed hit, the erratum note exists and names T1, the two downstream dependency lists match the index, the public-suffix backlog entry names this spec as the enforcement owner, and the status lint is green.
