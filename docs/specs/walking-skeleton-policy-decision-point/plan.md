...
**Touches:** src/ced/agents/toolsets/policy.py, src/ced/agents/toolsets/step_events.py, src/ced/agents/toolsets/trust_class.py, src/ced/agents/ceilings.py, src/ced/agents/compiler.py, src/ced/adapters/postgres/event_log.py, tests/authorization/**, tests/compiler/test_whole_tool_surface_refuses.py, tests/compiler/test_decision_point_refuses.py
...
- The four files added to `Touches` by the amendment of 2026-09-23 carry no criterion of their own; each exists because a criterion above cannot be met without it. `src/ced/agents/ceilings.py` decodes a stored `ceiling` entry's `predicates` into the containment fragment's `CeilingEntry`, which is the encoding revision 0003 left out of scope and `walking-skeleton-authority-containment` shipped without — AC-0207, AC-0210, AC-0235 and AC-0236 all need it to exist. `src/ced/agents/compiler.py` installs the resolver that module builds, because a predicate nothing wires in is a predicate no compiled stack holds. `src/ced/adapters/postgres/event_log.py` gains `terminate_step`, the fenced write AC-0211, AC-0243 and AC-0252 require and which must live beside `Fenced` in the adapter rather than as raw SQL in the agents layer. `tests/compiler/test_decision_point_refuses.py` loses one case: it builds a decision point with an admitting resolver and **no step context** and asserts the tool body runs, which AC-0209 retires — an admitted call now commits a `policy.decision` first, so an unbound decision point must refuse. AC-0234's own assertions in that file are untouched and stay green.
