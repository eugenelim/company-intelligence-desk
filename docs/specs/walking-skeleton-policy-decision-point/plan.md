...
- The two architecture documents added to `Touches` by the amendment of 2026-09-23 carry **ownership corrections only**. `role-configuration-seams.md` attributes the decision point's predicate to `walking-skeleton-authority-containment` in three places, and `runtime-architecture.md` r8 § 10 names that spec as the owner of the authorization boundary's encoding and evaluation; both are what the grep gate reads, and neither is reachable from any other task's `Touches`. **No Built or Designed status moves in r8.** § Durable Outputs routes the current-state map to `docs/architecture/README.md` and T3 to r5's STATUS header, so a status edit in r8 would change a ratified point-in-time record that this spec deliberately does not own.
...
**Done when:** the grep gate returns no misattributed hit — including in the two architecture documents the amendment of 2026-09-23 added, whose ownership clauses it reaches and no other task can — the erratum note exists and names T1, the two downstream dependency lists match the index, the public-suffix backlog entry names this spec as the enforcement owner, neither that entry nor `walking-skeleton-authority-containment`'s § Follow-ons still says the enforcement is unowned, and the status lint is green.
...
- 2026-09-23, amendment round 5: **T2's `Touches` was short of the two architecture documents its own grep gate reaches.** `docs/architecture/role-configuration-seams/role-configuration-seams.md` attributes the decision point's predicate to `walking-skeleton-authority-containment` in three places, and `docs/architecture/inspectable-multi-agent-diligence/runtime-architecture.md` r8 § 10 names that spec as owner of the authorization boundary's encoding and evaluation. Both postdate nothing — they are correct as written before the cut of 2026-09-23 and wrong after it — and neither is in any task's `Touches`, so T2's `Done when` could not be met inside the approved contract. Authorized by eugenelim on 2026-09-23, on the hand-reset route — Approved to Drafting, amend, re-approve — the same shape rounds 1 to 4 took and for the same recorded reason: `no-lifecycle-route-for-widening-an-approved-unsealed-contract` in `[backlog].open` still records that no documented transition covers widening an approved contract. **The engine's `contract-amendment` event is now reachable**, this run having entered `CODE-IMPLEMENTATION`, and was declined on its own terms: it exists to separate an accepted item that implementation proved separable, it clears the approval and schedule baseline and requires the pre-EXECUTE reviews and both human gates to run again, and nothing here is being separated or narrowed. **This is a fifth precedent, not a sanctioned route**, and it is recorded as one. The acceptance-criterion set is **unchanged**. **Declined:** widening the edits to move r8 § 10's Built/Designed status for the authorization boundary, because § Durable Outputs routes the current-state map to `docs/architecture/README.md` and T3 to r5's STATUS header, so r8's status is a ratified point-in-time record this spec does not own and § Agent Rules puts behind Ask-first; and leaving r8 alone and recording the surviving grep hit in the verification ledger, because the gate is T2's whole contract and a `Done when` met by exception is a gate that stops reading.
...
- 2026-09-23: plan re-approved by eugenelim after amendment round 5
